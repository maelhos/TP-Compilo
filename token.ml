...
and lexW (code : char list) (token : string) (liste_tokens : token list) : token list =
        ...
        |t::q when isspecial t -> tokenize code ((VAR token)::liste_tokens)
        |_::q -> failwith "invalid argument name"
and lexZ (code : char list) (token : string) (liste_tokens : token list) : token list =
        ...
        |t::q when isspecial t -> tokenize code ((CONST (int_of_string token))::liste_tokens)
        |_::q -> failwith "invalid number";;
...
print_tokenlist (lex (explode "[ xaxa, y ] xaxa + y * (xaxa - y)"));;
print_newline ();;
print_tokenlist (lex (explode "[ x ] x * 9"));;
